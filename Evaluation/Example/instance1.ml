 ...
 struct
  ...
  module Lex =
   struct
    ...
    let rec lex' rules code =
      let (label, o) = MPref.max_of_prefs (MPref.max_prefs code rules) in
      (match o with
       | Some p ->
         let (p0, suffix) = p in
         (match p0 with
          | [] -> ([], code)
          | ph :: pt ->
            let (lexemes, rest) = lex' rules suffix in
    ...
    let lex rules code =
      let srules = map init_srule rules in lex' srules code
   end
 end
 ...
 struct
   ...
   struct
    ...
    let rec max_pref_fn__M m s state =
      ...
      | a :: s' ->
        ...
        let (m', o) = mpxs in
        (match o with
         | Some p0 -> let (p, q) = p0 in (m', (Some ((a :: p), q)))
         | None ->
           if MEM.STT.Ty.accepting state'
           then (m', (Some ((a :: []), s')))
           else if MEM.STT.Ty.accepting state
                then (m', (Some ([], s)))
                else ((MEMO.set_Memo m' (fst state) s None), None))
    ...
    let extract_fsm_for_max__M code = function
    | (m, p) ->
      let (a, fsm) = p in
      let (m', o) = max_pref_fn__M m code fsm in (m', (a, o))
    ...
    let max_prefs__M ms code erules =
      let zipped = Utils.zip ms erules in
      let mapped = map (extract_fsm_for_max__M code) zipped in
      Utils.unzip mapped
    ...
    let rec max_of_prefs__M = function
    | (ms, prefs) ->
      let (l, o) = Defs.NaiveLexer.MPref.max_of_prefs prefs in ((ms, l), o)
   end
  ...
  module Lex =
   struct
    ...
    let rec lex'__M ms rules code =
      let (p, o) = MPref.max_of_prefs__M (MPref.max_prefs__M ms code rules) in
      let (ms', label) = p in
      (match o with
       | Some p0 ->
         let (p1, suffix) = p0 in
         (match p1 with
          | [] -> ((ms', []), code)
          | ph :: pt ->
            let (p2, rest) = lex'__M ms' rules suffix in
    ...
    let lex__M rules code =
      let srules = map init_srule rules in
      let (p, rest) = lex'__M (init_Memos srules) srules code in
...
module FMemo =
 ...
 struct
  ...
  let get_Memo m pnt sx =
    if Hashtbl.mem m (pnt, sx)
    then Some (Hashtbl.find m (pnt, sx))
    else None
 end
